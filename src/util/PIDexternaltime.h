...
class PIDexternaltime {
  ...
  public:
    PIDexternaltime(PIDparameters* pid) : param(pid), setPoint(0), previous(0), acc(0), stopped(true) {}
    ...
    void train(float out) { acc = constrain(out, param->lowerBound, param->upperBound); }
    ...
    float update(float current, float ms) {
        ...
        const float output = param->P * error + newAcc + param->D * (previous - current) / dt;
