...
float Quaternion::dot(const Quaternion& l) const { return w * l.w + x * l.x + y * l.y + z * l.z; }
float Quaternion::length() const { return sqrt(w * w + x * x + y * y + z * z); }
Vec3 Quaternion::axis() const { return Vec3(x, y, z); }
/**
 ...
 */
Vec3 Quaternion::getDerivative(Quaternion l) const {
    Quaternion der = l;
    der.rotateBy(~(*this));
...
Vec3 Quaternion::getDerivative(Quaternion l, float dt) const {
    Quaternion der = l;
...
float Quaternion::getPitch() const { return asin(2 * ((w * y) - (z * x))); }
float Quaternion::getRoll() const { return asin(2 * ((y * z) + (x * w))); }
float Quaternion::getYaw() const { return atan2(2 * ((z * w) - (x * y)), 1 - 2 * ((z * z) + (y * y))); }
...
void Quaternion::rotateBy(const Quaternion& l) {
...
void Quaternion::integrate(const Vec3& rotVel) {
...
void Quaternion::preintegrate(const Vec3& rotVel) {
...
Quaternion Quaternion::operator~(void) const { return this->inverse(); }
...
void Quaternion::operator*=(const Quaternion& r) { rotateBy(r); }
...
Quaternion operator*(const Quaternion& l, float s) { return Quaternion(l.w * s, l.x * s, l.y * s, l.z * s); }
Quaternion operator*(float s, const Quaternion& l) { return Quaternion(l.w * s, l.x * s, l.y * s, l.z * s); }
Quaternion operator/(const Quaternion& l, float s) { return Quaternion(l.w / s, l.x / s, l.y / s, l.z / s); }
