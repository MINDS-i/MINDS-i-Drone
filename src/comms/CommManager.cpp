...
CommManager::CommManager(HardwareSerial* inStream, Storage<float>* settings)
    : stream(inStream), bufPos(0), storage(settings), cachedTarget(0, 0), isLooped(false), targetIndex(0),
      waypointsLooped(false), connectCallback(NULL), eStopCallback(NULL), stateStopCallback(NULL),
      stateStartCallback(NULL), bumperDisableCallback(NULL), bumperEnableCallback(NULL), versionCallback(NULL),
      chksumFailureCount(0), pktMismatchCount(0), pktFormatErrorCount(0), pktDataCount(0), pktDataLen(0),
      pktDecodeState(READ_HEADER1) {
    waypoints = new SRAMlist<Waypoint>(MAX_WAYPOINTS);
}

void CommManager::defaultVersionCallback() { sendVersion(version_major, version_minor, version_rev); }
...
void CommManager::handleString(uint8_t* msg, uint8_t length) { /*dead end for strings*/
}
...
void CommManager::clearWaypointList() { waypoints->clear(); }
...
uint16_t CommManager::numWaypoints() { return waypoints->size(); }
...
void CommManager::advanceTargetIndex() { setTargetIndex(getTargetIndex() + 1); }

void CommManager::retardTargetIndex() { setTargetIndex(getTargetIndex() - 1); }

uint16_t CommManager::getTargetIndex() { return targetIndex; }

Waypoint CommManager::getTargetWaypoint() { return cachedTarget; }
...
void CommManager::inputSetting(uint8_t id, float input) { storage->updateRecord(id, input); }

float CommManager::getSetting(uint8_t id) { return storage->getRecord(id); }

void CommManager::requestResync() { sendSyncMessage(Protocol::SYNC_REQUEST); }

void CommManager::setConnectCallback(void (*call)(void)) { connectCallback = call; }

void CommManager::setEStopCallback(void (*call)(void)) { eStopCallback = call; }

void CommManager::setStateStopCallback(void (*call)(void)) { stateStopCallback = call; }

void CommManager::setStateStartCallback(void (*call)(void)) { stateStartCallback = call; }

void CommManager::setBumperDisableCallback(void (*call)(void)) { bumperDisableCallback = call; }

void CommManager::setBumperEnableCallback(void (*call)(void)) { bumperEnableCallback = call; }

void CommManager::setSettingsResetCallback(void (*call)(void)) { settingsResetCallback = call; }

void CommManager::setVersionCallback(void (*call)(void)) { versionCallback = call; }
...
void CommManager::sendTelem(uint8_t id, float value) {
    ...
    data.f = value;
    byte tmp[6] = {
        buildMessageLabel(dataSubtype(TELEMETRY)), id, data.bytes[3], data.bytes[2], data.bytes[1], data.bytes[0],
    };
...
void CommManager::sendState(uint8_t stateTypeId, uint8_t stateID) {
    byte tmp[3] = {
        buildMessageLabel(wordSubtype(STATE)),
        stateTypeId,
        stateID,
    };
...
void CommManager::sendSensor(uint8_t sensorTypeId, uint8_t sensorNum, uint32_t value) {
    byteConv data;
    data.l = value;
    byte tmp[5] = {buildMessageLabel(dataSubtype(SENSORS)), sensorTypeId, sensorNum, data.bytes[1], data.bytes[0]};
...
void CommManager::sendVersion(uint8_t version_major, uint8_t version_minor, uint8_t version_rev) {
    byteConv data;
    // data.l = value;
    byte tmp[5] = {buildMessageLabel(dataSubtype(INFO)), APM_VERSION, version_major, version_minor, version_rev};
    Protocol::sendMessage(tmp, 5, stream);
}
...
void CommManager::sendSetting(uint8_t id, float value) {
    ...
    data.f = value;
    byte tmp[6] = {
        buildMessageLabel(dataSubtype(SETTING)), id, data.bytes[3], data.bytes[2], data.bytes[1], data.bytes[0],
    };
    Protocol::sendMessage(tmp, 6, stream);
}
...
void CommManager::sendString(int type, const char* msg, uint8_t len) {
    ...
    Protocol::sendStringMessage(buildMessageLabel(stringSubtype(type)), msg, len, stream);
}
