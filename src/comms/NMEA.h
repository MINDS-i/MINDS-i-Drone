...
class NMEA {
  public:
    explicit NMEA(Stream& stream) : inStream(stream) {
        stream.setTimeout(0);
        memset(&curGPSCoord, 0, sizeof(GPS_COORD));
    }
    ...
    bool getUpdatedRMC() { return updatedRMC; }

    void clearUpdatedRMC() { updatedRMC = false; }
