...
class UM7 {
  ...
  public:
    ...
    UM7(HardwareSerial* inStream, void (*callback)(uint8_t, uint32_t)) : stream(inStream), updateData(callback) {}
...
void UM7::sendPacket(uint8_t PT, uint8_t ADDR, uint8_t* data, uint8_t len) {
    ...
    for (int i = 0; i < UM7::HEADER_SIZE; i++) {
        check += UM7::PACKET_HEADER[i];
    }
...
void UM7::sendPacket(uint8_t PT, uint8_t ADDR) { sendPacket(PT, ADDR, (uint8_t*)0, 0); }
...
void UM7::update() {
    while (stream->available()) {
        ...
        if (batch == 0 && (buffer[0] & 0x80) != 0) {
            batch = 1; // implied batch of one
        }
        uint8_t expectedSize = 4 + 4 * batch;

        if (bufferIndex >= UM7::MAX_BUFFER) {
            bufferIndex = 0;
        } else if (bufferIndex == expectedSize) {
            parsePacketTo(buffer, bufferIndex);
        }
    }
}

bool UM7::parsePacketTo(uint8_t* buf, uint8_t size) {
    uint16_t check = 0;
    for (int i = 0; i < UM7::HEADER_SIZE; i++) {
        check += UM7::PACKET_HEADER[i];
    }
    ...
    if (batch == 0 && size == 8) {
        batch = 1; // No batch, just one address
    }
    splitData(buf[1], batch, buf + 2); // Send the buffer starting at the data
    return true;
}

void UM7::splitData(uint8_t address, uint8_t batch, uint8_t* buf) {
