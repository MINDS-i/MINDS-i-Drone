...
class InertialManager {
  ...
  public:
    InertialManager(InertialVec** s, Translator* ts, uint8_t num) : sensor(s), translator(ts), numSensors(num) {}
    void update() {
        for (int i = 0; i < numSensors; i++) {
            sensor[i]->update(*this, translator[i]);
        }
    }
    Vec3 getGyro() { return gyro; }
    const Vec3* gyroRef() { return &gyro; }
    Vec3 getAccl() { return accl; }
    const Vec3* acclRef() { return &accl; }
    Vec3 getMag() { return mag; }
    const Vec3* magRef() { return &mag; }
