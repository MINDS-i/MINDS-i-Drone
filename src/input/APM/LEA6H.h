...
class LEA6H : public Sensor, public GPS {
  protected:
    ...
    void sendUBloxMessage(uint8_t Type, uint8_t ID, uint16_t len, const uint8_t* buf);
    void calcChecksum(const uint8_t* msg, uint8_t len, uint8_t& c_a, uint8_t& c_b);
  ...
  public:
    ...
    bool getWarning() {
        update();
        return parser.getWarning();
    }
    bool getUpdatedRMC() {
        update();
        return parser.getUpdatedRMC();
    }
    void clearUpdatedRMC() { parser.clearUpdatedRMC(); }
    uint16_t dataIndex() {
        update();
        return parser.dataIndex();
    }
    float getCourse() {
        update();
        return parser.getCourse();
    }
    float getDateOfFix() {
        update();
        return parser.getDateOfFix();
    }
    float getGroundSpeed() {
        update();
        return parser.getGroundSpeed();
    }

    GPS_COORD getGPS_COORD() {
        update();
        return parser.getGPS_COORD();
    }

    float getLatitude() {
        update();
        return parser.getLatitude();
    }
    float getLongitude() {
        update();
        return parser.getLongitude();
    }

    float getMagVar() {
        update();
        return parser.getMagVar();
    }
    float getTimeOfFix() {
        update();
        return parser.getTimeOfFix();
    }
    uint16_t getNumSat() {
        update();
        return parser.getNumSat();
    }
    float getHDOP() {
        update();
        return parser.getHDOP();
    }
#ifdef WAYPOINT_H
...
Sensor::Status LEA6H::status() { return Sensor::OK; }
void LEA6H::begin() {
...
void LEA6H::calibrate() {}
void LEA6H::update() { parser.update(); }
void LEA6H::sendUBloxMessage(uint8_t Type, uint8_t ID, uint16_t len, const uint8_t* buf) {
    uint8_t header[4];
    uint8_t check[2] = {0, 0};
    header[0] = Type;
    header[1] = ID;
...
const uint8_t LEA6H::GPS_RESET[] = {0x00, 0x00, 0x01, 0x00};
...
//                         0x00, 0x00, 0x00, 0x96, 0x00, 0x00, 0x07, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x82,
//                         0x8A};
const uint8_t LEA6H::Pedestrian_Mode[] = {0xFF, 0xFF, 0x03, 0x03, 0x00, 0x00, 0x00, 0x00, 0x10, 0x27, 0x00, 0x00,
...
const uint8_t LEA6H::CFG_PRT[] = {0x01, 0x00, 0x00, 0x00, 0xc0, 0x08, 0x00, 0x00, 0x00, 0x96,
                                  0x00, 0x00, 0x07, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00};
