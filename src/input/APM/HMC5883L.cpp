...
void HMC5883L::tune(LTATune t) { LTA = t; }
void HMC5883L::end() {}
bool HMC5883L::checkGoodValues() {
...
Sensor::Status HMC5883L::status() {
    ...
    return Sensor::OK; // always return sensor ok here, this function is called when board is powered on, good values
                       // will be checked at arming time.
}
void HMC5883L::calibrate() {}
void HMC5883L::update(InertialManager& man, Translator axis) {
    int m[3];
    rawValues(m[0], m[1], m[2]);
    float M[3];
...
void HMC5883L::rawValues(int& x, int& y, int& z) {
    Wire.beginTransmission(address);
