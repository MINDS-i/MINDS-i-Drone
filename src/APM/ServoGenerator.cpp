...
uint16_t constexpr intervalFromMicros(uint16_t us) { return us * TICKS_PER_MS; }

uint16_t constexpr microsFromInterval(uint16_t ticks) { return ticks / TICKS_PER_MS; }
} // namespace

namespace ServoGenerator {
bool begun;

void set(uint8_t channel, uint16_t us) { output[channel].highTime = intervalFromMicros(us); }
...
void setUpdateCallback(UpdateFunc callback) {
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE) { frameCallback = callback; }
}
...
ISR(TIMER_ISR(TIMER_NUM, CAPT)) {
    ...
    if (frameCallback != NULL && IN_FRAME_CALLBACK == false) {
        ...
        NONATOMIC_BLOCK(NONATOMIC_FORCEOFF) { frameCallback(microsFromInterval(ICR)); }
