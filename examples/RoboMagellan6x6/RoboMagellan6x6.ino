...
enum APM_STATES { APM_STATE_INVALID = 0, APM_STATE_INIT, APM_STATE_SELF_TEST, APM_STATE_DRIVE };
...
enum AUTO_STATES { AUTO_STATE_INVALID = 0, AUTO_STATE_FULL, AUTO_STATE_AVOID, AUTO_STATE_STALLED };
...
const char AVOID_STATES_STRING[AVOID_STATE_NUM_STATES][32] = {"Avoid State Enter",         "Avoid State Forward Brake",
                                                              "Avoid State Straightback",  "Avoid State Steer",
                                                              "Avoid State Reverse Brake", "Avoid State Done"};
...
struct schedulerData scheduler[] = {{extrapPosition, 110, 0, 1},    {reportLocation, 110, 44, 1},
                                    {reportState, 110, 110, 1},     {checkPing, 22, 88, 1},
                                    {checkBumperSensor, 80, 55, 1}, {checkVoltage, 100, 66, 1},
//                                  {navigate, 0, 66, 1},
#ifdef simMode
                                    {updateSIM, 500, 100, 1}
#endif
...
void pingBlockLevelEdgesCallback(float in) { blockLevel[0] = blockLevel[4] = (int)in; }
void pingBlockLevelMiddlesCallback(float in) { blockLevel[1] = blockLevel[3] = (int)in; }
void pingBlockLevelCenterCallback(float in) { blockLevel[2] = (int)in; }
void pingWarnLevelEdgesCallback(float in) { warnLevel[0] = warnLevel[4] = (int)in; }
void pingWarnLevelMiddlesCallback(float in) { warnLevel[1] = warnLevel[3] = (int)in; }
void pingWarnLevelCenterCallback(float in) { warnLevel[2] = (int)in; }
...
// testing
void triggerGyroSyncNorth(float in) { euler_z_offset = mpudmp.getEulerZ(); }
...
void setAutoStateFlag(uint8_t flag) { autoStateFlags |= flag; }

void clearAutoStateFlag(uint8_t flag) { autoStateFlags &= ~flag; }

bool isSetAutoStateFlag(uint8_t flag) { return (autoStateFlags & flag) > 0 ? true : false; }
...
void checkRadioFailsafe() {
    if (!radioFailsafeEnabled) {
        // captures the case when we're in failsafe and disable radio failsafe checks
...
void navigate() {
    ...
    if (driveState == DRIVE_STATE_AUTO) {
        if (autoState == AUTO_STATE_AVOID) {
            ...
            outputAngle = steering_control_lat_lon(backWaypoint.m_gpsCoord, manager.getTargetWaypoint().m_gpsCoord,
                                                   location.m_gpsCoord, (float)trueHeading);
            ...
            outputAngle = constrain(outputAngle, double(steerCenter - steerThrow), double(steerCenter + steerThrow));
...
void wgs84_to_cartesian(float& x, float& y, const GPS_COORD& coord, const GPS_COORD& ref_coord, float alt0) {
    ...
    double rho_e = equator_radius * (1.0 - eccentricity * eccentricity) / (sqrt(p) * p);
...
void positionChanged() {
    ...
    /*
        ...
        else
        {
            ...
            //  From there determine an intermediary waypoint along that original (optimal) vector is a lookAheadDis
       scalled by lineGravity.
...
void checkPing() {
    ...
    if (driveState == DRIVE_STATE_AUTO && autoState == AUTO_STATE_AVOID &&
        (avoidState == AVOID_STATE_REV_BRAKE || avoidState == AVOID_STATE_STEER_FWD)) {
    ...
    {
        if (ping[2][PING_CUR] < warnLevel[2] &&
            (ping[0][PING_CUR] >= warnLevel[0] && ping[1][PING_CUR] >= warnLevel[1] &&
             ping[3][PING_CUR] >= warnLevel[3] && ping[4][PING_CUR] >= warnLevel[4])) {
...
void updateHeading() {
    ...
    {
        // AsciiMsg_t msg;
        // String tst = String(millis()) + ": HEADING_MSG : heading: " + String(trueHeading, 4) + " imu_yaw: " +
        // String(mpudmp.getEulerZ() * 180.0 / M_PI, 4) + " ratio: " + String(ratio_adjustment, 10); msg.ascii.len =
        // tst.length(); tst.toCharArray(msg.ascii.data,tst.length()+1); debugger.send(msg);
    } {
        // AsciiMsg_t msg;
        // String tst = String(millis()) + ": IMU_MSG_RPY:  r: " + String(mpudmp.getEulerX()) + "  p: " +
        // String(mpudmp.getEulerY()) + "  y: " + String(mpudmp.getEulerZ()) + '\n' +
        //              String(millis()) + ": IMU_MSG_ACC: ax: " + String(mpudmp.get_ax()) + " ay: " +
        //              String(mpudmp.get_ay()) + " az: " + String(mpudmp.get_az()) + '\n' +
        //                   String(millis()) + ": IMU_MSG_ANG: wx: " + String(mpudmp.get_wx()) + " wy: " +
        //                   String(mpudmp.get_wy()) + " wz: " + String(mpudmp.get_wz());
        // msg.ascii.len = tst.length();
        // tst.toCharArray(msg.ascii.data,tst.length()+1);
        // debugger.send(msg);
    }
    ...
    if (new_gps) {
        ...
        if (fabs(cur_wheel_speed) > speed_thresh && gps_heading_init &&
            !isSetAutoStateFlag(AUTO_STATE_FLAG_TURNAROUND) && driving_straight) {
            ...
            cur_heading_est = truncateDegree(
                (cur_heading_est / cur_heading_variance + (cur_heading_est + heading_diff) / gps_heading_var) /
                var_denominator);
...
void updateGyro() {
    ...
    if (ctr >= 10) {
        ...
        msg.pitch =
            toDeg((mpudmp.getEulerX() * cos(mpudmp.getEulerZ()) - mpudmp.getEulerY() * sin(mpudmp.getEulerZ())));
        debugger.send(msg);
        ctr = 0;
    } else {
        ctr++;
...
void reportLocation() {
    ...
    manager.sendTelem(Protocol::telemetryType(PITCH), toDeg((mpudmp.getEulerX() * cos(mpudmp.getEulerZ()) -
                                                             mpudmp.getEulerY() * sin(mpudmp.getEulerZ()))));
    manager.sendTelem(Protocol::telemetryType(ROLL), toDeg((mpudmp.getEulerY() * cos(mpudmp.getEulerZ()) +
                                                            mpudmp.getEulerX() * sin(mpudmp.getEulerZ()))));
...
void updateSteerSkew(float s) { steerSkew = int8_t(round(s)); }
...
void newPIDparam(float x) {
    ...
    //		 [900,2100] PWM
    cruisePID = PIDparameters(settings.get(11), settings.get(12), settings.get(13), -60, 60);
}

void stateStop() { changeDriveState(DRIVE_STATE_STOP); }
...
void bumperEnable() { isBumperEnabled = true; }

void bumperDisable() { isBumperEnabled = false; }
...
void setupSettings() {
     ...
     */
    index = 0;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<float, &lineGravity>);
    ...
    index = 1;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<int, &steerThrow>);
    ...
    index = 2;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<int, &steerStyle>);
    ...
    index = 3;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<float, &steerFactor>);
    ...
    index = 4;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<float, &minFwd>);
    ...
    index = 5;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<float, &maxFwd>);
    ...
    index = 6;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<int, &revThrow>);
    ...
    index = 7;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<float, &revSpeed>);
    ...
    index = 8;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<float, &pingWeight>);
    ...
    index = 9;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<int, &avoidCoastTime>);
    ...
    index = 10;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), &dangerTimeCallback);
    ...
    index = 11;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), &newPIDparam);
    ...
    index = 14;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<float, &tireDiameter>);
    ...
    index = 15;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<int, &steerCenter>);
    ...
    index = 16;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<float, &PointRadius>);
    ...
    index = 17;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), callback<float, &approachRadius>);
    ...
    index = 20;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), &updateSteerSkew);
    ...
    index = 21;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), &pingBlockLevelEdgesCallback);
    ...
    index = 22;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), &pingBlockLevelMiddlesCallback);
    ...
    index = 23;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), &pingBlockLevelCenterCallback);
    ...
    index = 24;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), &pingWarnLevelEdgesCallback);
    ...
    index = 25;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), &pingWarnLevelMiddlesCallback);
    ...
    index = 26;
    settings.attach(index, pgm_read_float_near(&settingsData[index][0]), pgm_read_float_near(&settingsData[index][1]),
                    pgm_read_float_near(&settingsData[index][2]), &pingWarnLevelCenterCallback);
